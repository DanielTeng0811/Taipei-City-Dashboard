...
def generate_geojson():
    try:
        ...
        def save_geojson(data_list, prefix):
            taipei_features = []
            metro_features = []
            for r in data_list:
                feat = {
            ...
            with open(f'../Taipei-City-Dashboard-FE/public/mapData/{prefix}_taipei.geojson', 'w', encoding='utf-8') as f:
                json.dump({"type": "FeatureCollection", "features": taipei_features}, f, ensure_ascii=False, indent=2)
            with open(f'../Taipei-City-Dashboard-FE/public/mapData/{prefix}_metro.geojson', 'w', encoding='utf-8') as f:
                json.dump({"type": "FeatureCollection", "features": metro_features}, f, ensure_ascii=False, indent=2)
            print(f"Generated {prefix}_taipei.geojson and {prefix}_metro.geojson")

        save_geojson(zone2_map, 'eco_zone2')
        save_geojson(zone3_map, 'eco_zone3')
        save_geojson(zone4_map, 'eco_zone4')
